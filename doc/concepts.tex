...
(the element index) starting from one. The first element is called \vn{beginning_ele} (\sref{s:begin.ele}).
...
Chapter~\sref{c:sequence} shows how a \vn{branch} can be defined using f \vn{line}s.
...
There are two types of \vn{branches}, if \vn{Branch.type}  \eeeee
