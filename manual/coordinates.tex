...
a ``\vn{machine}'' coordinate system is used (\sref{s:local.coords}).  This curvilinear coordinate
...
body}'' (or just ``\vn{body}'') 
coordinates which are attached to the physical element. That is, the electric and magnetic
fields of an element are described with respect to \vn{body} coordinates.  If there are no
misalignments, the \vn{body} coordinates are aligned with \vn{local} coordinates which
are the same as \vn{machine} coordinates except that $s = 0$ in \vn{local} coordinates is at the beginning of the non-misaligned
...
not ``misaligned'' is the same as the \vn{local} coordinates. 
\vn{Local} coordinates are the same as \vn{machine} coordinates
except that $s = 0$ in \vn{local} coordinates is at the beginning of the non-misaligned
