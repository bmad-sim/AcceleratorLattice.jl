...
not ``misaligned'' is the same as the \vn{machine} coordinates. The $x=y=0$ curved line of the
machine coordinate system is known as the ``reference orbit''. For each lattice element,
there is also a \vn{local} coordinate system which is the same as the \vn{machine} coordinate
system except that $s = 0$ in \vn{local} coordinates is at the beginning of the non-misaligned
...
Note: The old Fortran \bmad names for the coordinate systems was somewhat different and not
