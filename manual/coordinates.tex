...
\vn{Local} coordinates are the same as \vn{machine} coordinates
except that $s = 0$ in \vn{machine} coordinates is at the beginning of the non-misaligned
element and $s = 0$ in machine coordinates is at the beginning
of the line. The $x=y=0$ curved line of the
machine coordinate system is known as the ``reference orbit''.}
  \label{f:coords}
\end{figure}

It is not convenient to describe the position of a particle beam using the \vn{global} coordinate system so
...
position and orientation shifts (\sref{s:align.g}) which are sometimes referred to as ``misalignments''. 
Each lattice element has ``\vn{element body}'' (or just ``\vn{body}'') 
coordinates which are attached to the physical element. The electric and magnetic
...
\vn{local} coordinates except that $s = 0$ in \vn{machine} coordinates is at the beginning of the non-misaligned
element and $s = 0$ in machine coordinates is at the beginning of the line.
...
\begin{figure}[tb]
  ...
  \caption[The machine Reference System.]
{The machine reference coordinate system. By construction, a particle's $z$ coordinate is zero.  This
...
\vn{reversed} (\fig{f:machine.coords}B). The $x$ and $y$--axes are perpendicular to the reference
orbit and, by construction, the particle is always at $z = 0$. The coordinate system so constructed
is called the \vn{``local coordinate system''} or sometimes the \vn{``machine coordinate
system''} when there is need to distinguish it from the \vn{``element body coordinate system''}
...
centered with respect to the reference curve. However, by specifying offsets, rotations (See
...
Normally, the \vn{upstream} end is the same as the element's \vn{entrance} end (\fig{f:ele.coord.frame}) and the
...
Also the orientation of an element depends only on the \vn{x_rot}, \vn{y_rot}, \vn{roll}, and \vn{ref_tilt}, and is
