...
called ``\vn{floor}'') coordinates are independent of the placement of the machine. Things such as the
...
system. Each lattice element has \vn{element body} coordinates which, if the element is
...
fields of an element are described with respect to \vn{body} coordinates.  If an element
has no
misalignments, the \vn{body} coordinates of the element are aligned with the 
...
a particle's position as shown in \fig{f:machine.coords}. The machine coordinate system is also used for
