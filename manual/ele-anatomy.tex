...
types which inherit from the abstract type \vn{EleParameterGroup}. To see a list of all
parameter groups, use the \vn{suptypes} function:
\begin{example}
  ...
  28-element Vector{Any}:
   ...
   ...
\end{example}
To see the parameter groups associated with a given element type, use the the \vn{info} function:
\begin{example}
  julia> info(Quadrupole)
...
\end{example}
Alternatively, when using the Julia REPL, in help mode typing the name of an element type will give
the same list.
