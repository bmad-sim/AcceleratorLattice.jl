...
\begin{example}
  ...
  misalignment_moves_aperture::Bool - Alignment affects aperture? Default false.
  custom_aperture::Dict             - Custom aperture information.
  aperture_at::BodyLoc.T - Aperture location. Default is BodyLoc.ENTRANCE_END.
\end{example}
...
\begin{example}
  Start at upstream end of element
  Check upstream aperture if there is one.
  Convert from machine coordinates to body coordinates.
  Track through the element body.
  Convert from body coordinates to machine coordinates.
  Check downstream aperture if there is one.
  End at downstream end of element.
\end{example}
...
\begin{example}
  section = WallSection([Vertex1([1.0, 4.0]), Vertex1([-1.0, 4.0, 6.0]), vertex1([-5.0, 1.0]),
                        Vertex1([-5.0, -1.0]), Vertex1([1.0, -1.5)]], r0 = [-2.5, 0.5]) 
\end{example}
...
\begin{example}
  ...
  println(qq.Bn0)               # But can use non-integrated component in an equation.
  qq.Bn0 = 0.7                  # Cannot set non-integrated component! Will throw an error!
  toggle_integrated!(qq, MAGNETIC, 0)  # toggle integrated setting for order 0 multipole.
\end{example}
...
set. For example:
\begin{example}
  @ele qq = Quadrupole(l = 0.6, Es0L = 1.0)  # 0th order multipole is integrated
  qq.En1 = 0.3                  # 1st order multipole is not integrated
  qq.Es1 = 0.5                  # This is OK.
  println(qq.Eintegrated0)      # Will print ``true''
  println(qq.En0)               # Can use non-integrated component in an equation.
  toggle_integrated!(qq, ELECTRIC, 0)  # toggle integrated setting for order 0 multipole.
\end{example}
